...
export function PatientPortal() {
  const [authMode, setAuthMode] = useState<"options" | "login" | "register" | "magic-link">("options");
    ...
    return (
      <div className="px-6 py-12 flex flex-col h-full bg-app-bg items-center justify-center text-center min-h-[85vh]">
        ...
        <p className="text-text-muted text-sm leading-relaxed mb-8 px-4 text-balance">
          Accede a tu cuenta para gestionar tus citas, unirte a videollamadas y descargar recursos.
        </p>
              ...
              >
                ...
                <div className="relative py-2">
                  ...
                  <div className="relative flex justify-center text-xs">
                    <span className="bg-app-bg px-2 text-text-muted">O ingresa con tu correo</span>
                  </div>
                </div>

                <button 
                  onClick={() => setAuthMode("login")}
                  className="w-full bg-primary text-white font-medium py-3 px-4 rounded-xl shadow-sm hover:bg-primary-dark transition-colors text-sm"
                >
                  Iniciar sesión
                </button>
                ...
                <button 
                  ...
                  className="w-full bg-white border border-border text-text-muted font-medium py-3 px-4 rounded-xl shadow-sm hover:bg-gray-50 transition-colors text-sm"
                >
                  Crear cuenta nueva
                </button>
                ...
                <button 
                  ...
                  className="text-xs text-primary underline mt-2 block w-full"
                >
                  Prefiero ingresar sin contraseña (Magic Link)
                </button>
              </motion.div>
            ) : (
              <motion.form
                ...
                className="bg-white p-6 rounded-2xl shadow-sm border border-border text-left"
              >
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-bold text-text-main">
                    {authMode === "login" ? "Iniciar sesión" : authMode === "register" ? "Crear cuenta" : "Link de acceso rápido"}
                  </h3>
                  <button type="button" onClick={() => { setAuthMode("options"); setAuthError(""); }} className="text-text-muted hover:text-text-main text-xs font-semibold">
                    Volver
                  </button>
                </div>

                {magicLinkSent ? (
                  <div className="bg-green-50 text-green-700 p-4 rounded-xl text-sm mb-4 text-center">
                    ¡Revisa tu bandeja de entrada! Te enviamos un enlace mágico para ingresar.
                  </div>
                ) : (
                  <>
                    <div className="space-y-4">
                      {authMode === "register" && (
                        <div>
                          <label className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-1 block">Nombre completo</label>
                          <input 
                            type="text" required value={name} onChange={e => setName(e.target.value)}
                            className="w-full border border-border rounded-lg p-3 text-sm focus:ring-2 focus:ring-primary/20 outline-none"
                      ...
                      <div>
                        <label className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-1 block">Correo Electrónico</label>
                        <input 
                      ...
                      {authMode !== "magic-link" && (
                        <div>
                          <label className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-1 block">Contraseña</label>
                          <input 
                    ...
                    <Button type="submit" disabled={authLoading} className="w-full mt-6 shadow-md">
